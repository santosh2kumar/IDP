...
def senddata(kproducer, csession, messageid, deviceid, devicetime, category, measure1, measure2, sendtime, patterncode):
    ...
    #write in Kafka
    if use_kafka:
        kproducer.send('flinktopic', str(data).encode('utf-8'))
...
def main():
    ...
    #connect to Kafka
    if use_kafka:
        kproducer = KafkaProducer(bootstrap_servers=['kafka-svc:9093'])
        if use_print:
            print("Connected a producer to Kafka servers: {}".format('kafka-svc:9093'))
    else:
        kproducer = None
